...
class Statistical(object):
  ...
  def showFrequencies1(self,dictionary):
    ...
    for c in sorted(self.stats1, key=self.stats1.get, reverse = True):
      result += "%s:%3d->%s" % (c, self.stats1[c],dictionary[c])
    return result

  def showFrequencies2(self,dictionary):
    result = ''
    for c in sorted(self.stats2, key=self.stats2.get, reverse = True):
      result += "%s:%2d->%s" % (c, self.stats2[c],dictionary[c[0:1]] + dictionary[c[1:2]])
    return result

  def showFrequencies3(self,dictionary):
    result = ''
    for c in sorted(self.stats3, key=self.stats3.get, reverse = True):
      result += "%s:%2d->%s" % (c, self.stats3[c],dictionary[c[0:1]] + dictionary[c[1:2]] + dictionary[c[2:3]])
    return result

  def accept(self, command,ciphertext, dictionary):
    if command[0:2] == 'f1':
      self.freq1(ciphertext)
      return (True, False, self.showFrequencies1(dictionary))
    elif command[0:2] == 'f2':
      self.freq2(ciphertext)
      return (True, False, self.showFrequencies2(dictionary))
    elif command[0:2] == 'f3':
      self.freq3(ciphertext)
      return (True, False, self.showFrequencies3(dictionary))
    return (False,False,'')
